...
public class IndexController extends BaseController {
    ...
    @RequestMapping(value = "anon/register")
    ...
    private ModelAndView LoginPost(HttpServletRequest request) {
        ...
        if (subject != null && subject.isAuthenticated() && subject.isRemembered()) {
            ...
            if (isAuthorized) {
                mv.setViewName(Constant.MAIN_PAGE);
                return mv;
            }
        }
    ...
    private String doLogout(Model model) {
        ...
        return Constant.LOGIN_PAGE;  // 建议在调用subject.logout后立即向终端重定向一个新的视图或页面。这样即能保证与安全相关的Cookie都能像预期的一样被删除
    }
