    ...
    $.sertab = {
        ...
        closeTab: function () {
            ...
            if ($(this).parents('.menuTab').hasClass('active')) {
                ...
                $.sertab.scrollToTab($('.menuTab.active'));
            }
            return false;
        },
        addTab: function () {
            $("#navbar-custom-menu").find('.ul>li.open').removeClass("open");
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.sertab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                scrollVal = $.sertab.calSumWidth($(tabElement).prevAll());
                if (scrollVal > 0) {
                    $('.page-tabs-content').animate({
        ...
        scrollTabLeft: function () {
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                offsetVal = 0;
                if ($.sertab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                    while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                        offsetVal += $(tabElement).outerWidth(true);
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.sertab.calSumWidth($(element).prevAll()), marginRightVal = $.sertab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.sertab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        init: function () {
            $('.menuItem').on('click', $.sertab.addTab);
            $('.menuTabs').on('click', '.menuTab i', $.sertab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.sertab.activeTab);
            $('.tabLeft').on('click', $.sertab.scrollTabLeft);
            $('.tabRight').on('click', $.sertab.scrollTabRight);
            $('.tabReload').on('click', $.sertab.refreshTab);
            ...
            $('.tabCloseOther').on('click', $.sertab.closeOtherTabs);
            $('.fullscreen').on('click', function () {
                if (!$(this).attr('fullscreen')) {
                    $(this).attr('fullscreen', 'true');
                    $.sertab.requestFullScreen();
                } else {
                    $(this).removeAttr('fullscreen')
                    $.sertab.exitFullscreen();
                }
            });
        }
    };
        ...
        loadMenu: function (json) {
            ...
            $.each(data, function (i) {
                ...
                if (row.supId == "0") {
                    ...
                    var childNodes = $.serindex.jsonWhere(data, function (v) { return v.supId == row.menuId });
                    if (childNodes.length > 0) {
                        _html += '<a href="' + row.menuUrl + '" class="dropdown-toggle">';
                        _html += '<i class="menu-icon fa ' + row.icon + '"></i><span class="menu-text"> ' + row.menuName + ' </span>';
                        _html += '<b class="arrow fa fa-angle-down"></b></a>';
                        _html += '<b class="arrow"></b>';
                        _html += '<ul class="submenu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.serindex.jsonWhere(data, function (v) { return v.supId == subrow.menuId });
                            if (subchildNodes.length > 0) {
                                _html += '<li class="">';
                                _html += '<a href="#" class="dropdown-toggle">';
                                _html += '<i class="menu-icon fa ' + subrow.icon + '"></i><span class="menu-text">' + subrow.menuName + '</span>';
                                _html += '<b class="arrow fa fa-angle-down"></b></a>';
                                _html += '<b class="arrow"></b>';
                                _html += '<ul class="submenu">';
            ...
            $("#sidebar-menu li a").click(function () {
                var d = $(this), e = d.next();
                if (e.is(".submenu") && e.is(":visible")) {
                    e.slideUp(500, function () {
                        e.removeClass("dropdown-toggle")
                    }),
                    e.parent("li").removeClass("active")
                } else if (e.is(".submenu") && !e.is(":visible")) {
                    var f = d.parents("ul").first(),
                    g = f.find("ul:visible").slideUp(500);
                    g.removeClass("dropdown-toggle");
                    var h = d.parent("li");
