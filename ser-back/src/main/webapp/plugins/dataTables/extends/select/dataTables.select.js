...
function info ( api )
{
	...
	var add = function ( name, num ) {
	    var nameCN = "";
	    if (name == 'row') nameCN = "行";
	    if (name == 'column') nameCN = "列";
	    if (name == 'cell') nameCN = "单元格";

		output.append( $('<span class="select-item"/>').append('&nbsp;&nbsp;').append( api.i18n(
			'select.'+name+'s',
			{ _: '%d '+nameCN+' 选中', 0: '', 1: '1 '+nameCN+' 选中' },
			num
		) ) );
	};
