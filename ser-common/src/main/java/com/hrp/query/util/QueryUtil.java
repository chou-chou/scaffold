package com.hrp.query.util;

import com.hrp.exception.QueryException;
import com.hrp.plugins.Page;
import com.hrp.query.entity.Column;
import com.hrp.query.entity.Query;
import com.hrp.query.entity.QueryCondition;
import com.hrp.query.filter.ConditionOperator;
import com.hrp.query.filter.IFilter;
import com.hrp.query.filter.StringFilter;
import com.hrp.query.pojo.QueryDefinition;
...
public class QueryUtil {
     ...
     * @return 分页信息
     */
    public static Page getPageInfo(QueryCondition queryCondition, Query query) {
        Page page = new Page();
        if (queryCondition.getPage() == null) {
            page.setTotalPage(query.getTotalPage());
        } else {
     ...
     */
    public static List queryByService(QueryCondition queryCondition, Query query, Page page) throws QueryException {
        ...
        try {
            Method method = clazz.getDeclaredMethod(query.getMethod(), QueryCondition.class, Page.class);
            return (List) method.invoke(service, queryCondition, page);
        } catch (Exception ex) {
    ...
    public static Type[] getTypeArr(Object[] objArr) {
        ...
        for (int i = 0; i < objArr.length; i++) {
            ...
            if (obj instanceof String) {
                ...
                typeArr[i] = Integer.class;
            } else if (obj instanceof Boolean) {
                typeArr[i] = Boolean.class;
            } else if (obj instanceof Date) {
                typeArr[i] = Date.class;
            } else {
                typeArr[i] = String.class;
            }
        }
    ...
    public static Map getSqlParams(QueryCondition queryCondition, Query query) throws QueryException{
        ...
        if (conPos > -1) {
            ...
            if (filter != null) {
                ...
                countMatch = StringUtil.countMatches(sql, replaced);
    ...
    private static String getSortInfo(QueryCondition condition, Query query) {
        return !StringUtil.isEmpty(condition.getSortInfo()) ? condition.getSortInfo() : query.getOrder();
    }
    ...
    private static String getOperatorStr(Map map, Column column) {
        if (map.get("operator") != null && !StringUtil.isEmpty(map.get("operator").toString())) {
    ...
    public static Object getValueByOperator(ConditionOperator operator, Object value, Class clazz) throws Exception {
        ...
        return JsonUtil.getValue(clazz, value);
    }
    ...
    private static Object getValueForBetween(Object value, Class clazz) throws Exception {
        ...
        for (String s : values) {
            if (StringUtil.isEmpty(s)) {
                ...
                list.add(JsonUtil.getValue(clazz, s));
            }
        }
        //只选取了开始时间
        if(values.length==1){
    ...
    private static List<Map<String, Object>> getServerConditions(Query query) {
        ...
        List<Map<String, Object>> serverConditions = new ArrayList<Map<String, Object>>();
    ...
    public static IFilter generateFilter(QueryCondition condition, Query query, StringBuilder sqlBuilder) throws QueryException {
        ...
        if (!StringUtil.isEmpty(sortInfo)) {
            ...
            if (query.getAllowPaging() && StringUtil.isNotBlank(query.getSortKey())) {
        ...
        IFilter filter = null;
        if (!StringUtil.isEmpty(query.getFilter())) {
        ...
        for (Map<String, Object> map : conditions) {
            ...
            Class clazz;
            try {
                clazz = JsonUtil.getClass(column.getClassType());
            } catch (Exception ex) {
