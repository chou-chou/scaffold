...
public class ExportUtil {
    ...
    public static void writeTitle(QueryCondition queryCondition, Query query, WritableSheet sheet) throws QueryException {
        try {
            //写标题
            Label title = new Label(0, 0, StringUtil.isEmpty(queryCondition.getSheetTitle()) ?
            ...
            //写副标题
            if (!StringUtil.isEmpty(queryCondition.getSheetSubTitle())) {
            ...
            for (int rowIndex = 1; rowIndex <= rowLen; ++rowIndex) {
                ...
                for (int colIndex = 0; colIndex < colLen; ++colIndex) {
                    ...
                    if (isExport(column)) {
                        ...
                        //根据类型设置单元格类型并写入数据
                        if (column.getClassType().equals("java.lang.Double") &&StringUtil.isEmpty(column.getSuffix())) {
                        ...
                        } else if (column.getClassType().equals("java.lang.Integer")&&StringUtil.isEmpty(column.getSuffix())) {
                        ...
                        if (!StringUtil.isEmpty(data) && data.getBytes().length > sheet.getColumnWidth(colIndex - ignoreCount))
